...
@push('script-page')
    ...
    <script>
        ...
        var showLeadUrl = "{{ route('deals.show', $deal->id) }}";
    ...
    {{-- <script>
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                // Fetch and update the full lead details in #main-content
                $.ajax({
                    url: showLeadUrl,
                    type: 'GET',
                    success: function (response) {
            ...
            error: function (xhr) {
                console.error(xhr.responseText);
                show_toastr('error', 'There was an error!');
            }
        });
    });

    // Function to rebind modal reset logic
    function rebindModalReset() {
        $('#commonModal').on('hidden.bs.modal', function () {
            $(this).find('.modal-title').html(''); // Clear title
            $(this).find('.body').html('');       // Clear body
            $(this).find('.modal-dialog').removeClass('modal-xl modal-md modal-sm'); // Reset size
        });
    }

    // Initial binding for the first page load
    rebindModalReset();
...
@section('breadcrumb')
    ...
    <div class="d-flex align-items-center mt-2">
        <a href="{{ route('deals.index') }}" class="text-black me-3">
            <i class="fas fa-arrow-left"></i> <!-- Font Awesome Left Arrow Icon -->
        </a>

        @if($deal->company_entity_logo)
        <img src="{{ asset('storage/app/public/' . $deal->company_entity_logo) }}" alt="Company Logo" style="width: 32px; height: 32px; object-fit: cover; border-radius: 4px;">
        @else
            <i class="fas fa-building fa-2x me-2"></i>
        @endif

        <div>
            <h3 class="d-inline">{{ $deal->company_entity_name }}</h3>
            <p class="d-inline ms-2">
                <a href="{{ $deal->company_website }}" target="_blank">{{ $deal->company_website }}</a>
            </p>
        </div>
    </div>
@endsection
@section('action-btn')
    <div class="float-end">
        ...
        <a href="#" data-size="lg" data-url="{{ route('deals.edit',$deal->id) }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{__('Edit')}}" data-title="{{__('Edit')}}" class="btn btn-sm btn-primary">
            <i class="ti ti-pencil"></i>
        </a>
        @if($previousDeal)
        <a href="{{ route('deals.show', $previousDeal->id) }}" class="btn btn-sm btn-primary" title="{{ __('Previous Lead') }}">
            <i class="fas fa-arrow-left"></i>
        </a>
    @endif

    @if($nextDeal)
        <a href="{{ route('deals.show', $nextDeal->id) }}" class="btn btn-sm btn-primary" title="{{ __('Next Lead') }}">
