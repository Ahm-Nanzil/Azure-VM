@if(isset($visit))
    {{ Form::model($visit, array('route' => array('customer.visit.update', $customer->id, $visit->id), 'method' => 'PUT','class' => 'modalForm')) }}
@else
    {{ Form::open(array('route' => ['customer.visit.store',$customer->id], 'class' => 'modalForm')) }}
@endif
    ...
    @foreach($visitFiles as $file)
        @if (file_exists(storage_path('lead_files/'.$file->file_path)))
            ...
            dropzoneBtn(mockFile, {
                download: "{{route('customer.file.download',[$customer->id,$file->id])}}",
                delete: "{{route('customer.file.delete',[$customer->id,$file->id])}}"
            });
        @endif
    @endforeach
@endif
    ...
    $('.summernote-simple').on('summernote.blur', function () {
        ...
        $.ajax({
            url: "{{route('customer.note.store',$customer->id)}}",
