...
@section('action-btn')
    <div class="float-end">
        ...
        <a href="{{ route('inventory.delivery-notes.create') }}" data-size="lg"   title="{{__('Delivery Note')}}" class="btn btn-sm btn-primary">
                    ...
                    <div class="table-responsive">
                        <div class="table-responsive">
                            <table class="table datatable">
                                <thead>
                                    <tr>
                                        <th>{{ __('Internal Delivery Note') }}</th>
                                        <th>{{ __('Deliverer') }}</th>
                                        <th>{{ __('Added From') }}</th>
                                ...
                                <tbody>
                                    @foreach ($deliveryNotes as $note)
                                        <tr class="font-style">
                                            <td>{{ $note->internal_delivery_name }}</td>
                                            <td>{{ $note->deliverer }}</td>
                                            <td>{{ $note->created_by->name ?? __('Unknown') }}</td> <!-- Assuming you have a relationship with the user -->
                                            <td>{{ $note->created_at->format('Y-m-d') }}</td>
                                            <td>{{ \Auth::user()->priceFormat($note->total_amount) }}</td>
                                            <td>{{ $note->status ? __('Active') : __('Inactive') }}</td>
                                            <td class="Action">
                                                ...
                                                <div class="action-btn bg-info ms-2">
                                                    <a href="{{ route('inventory.delivery-notes.edit', $note->id) }}" data-bs-toggle="tooltip" title="{{ __('Edit') }}">
                                                        <i class="ti ti-pencil text-white"></i>
                                                    </a>
                                                </div>
                                                <div class="action-btn bg-danger ms-2">
                                                    {!! Form::open(['method' => 'DELETE', 'route' => ['inventory.delivery-notes.destroy', $note->id], 'id' => 'delete-form-' . $note->id]) !!}
