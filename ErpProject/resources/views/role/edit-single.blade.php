{{Form::model($role,array('route' => array('roles.update.single', $role->id), 'method' => 'PUT')) }}
<div class="modal-body">
    <div class="row">
        <div class="col-lg-12 col-md-12 col-sm-12">
            <div class="form-group">
                {{Form::label('name',__('Name'),['class'=>'form-label'])}}
                {{ Form::text('name', null, array('class' => 'form-control', 'placeholder' => __('Enter Role Name'), 'readonly' => 'readonly')) }}
                @error('name')
            ...
            <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                ...
                <li class="nav-item">
                    <a class="nav-link" id="pills-inventory-tab" data-bs-toggle="pill" href="#inventory" role="tab" aria-controls="pills-contact" aria-selected="false">{{__('Inventory')}}</a>
                </li>
            ...
            <div class="tab-content" id="pills-tabContent">
                                        ...
                                        <tr class="details-row" style="display:none;">
                                            <td colspan="3">
                                                <div class="details-container">
                                                    <input type="checkbox" class="form-check-input align-middle ischeck crm_checkall" data-id="{{ str_replace(' ', '', $pipeline->name) }}">
                                                    <label class="ischeck crm_checkall" data-id="{{ str_replace(' ', '', $pipeline->name) }}">
                                                        {{ ucfirst($pipeline->name) }}
                                                    </label>
                                                    <div class="row">
                                                        @if(in_array('manage pipeline '.$pipeline->name,(array) $permissions))
                                                            @if($key = array_search('manage pipeline '.$pipeline->name,$permissions))
                                                                <div class="col-md-3 custom-control custom-checkbox">
                                                                    {{Form::checkbox('permissions[]',$key,$role->permission, ['class'=>'form-check-input crm_checkall isscheck_'.str_replace(' ', '', $pipeline->name),'id' =>'permission'.$key])}}
                                                                    {{Form::label('permission'.$key,'Manage',['class'=>'custom-control-label'])}}<br>
                                                                </div>
                                                            @endif
                                                        @endif
                                                        @if(in_array('create pipeline '.$pipeline->name,(array) $permissions))
                                                            @if($key = array_search('create pipeline '.$pipeline->name,$permissions))
                                                                <div class="col-md-3 custom-control custom-checkbox">
                                                                    {{Form::checkbox('permissions[]',$key,$role->permission, ['class'=>'form-check-input crm_checkall isscheck_'.str_replace(' ', '', $pipeline->name),'id' =>'permission'.$key])}}
                                                                    {{Form::label('permission'.$key,'Create',['class'=>'custom-control-label'])}}<br>
                                                                </div>
                                                            @endif
                                                        @endif
                                                        @if(in_array('edit pipeline '.$pipeline->name,(array) $permissions))
                                                            @if($key = array_search('edit pipeline '.$pipeline->name,$permissions))
                                                                <div class="col-md-3 custom-control custom-checkbox">
                                                                    {{Form::checkbox('permissions[]',$key,$role->permission, ['class'=>'form-check-input crm_checkall isscheck_'.str_replace(' ', '', $pipeline->name),'id' =>'permission'.$key])}}
                                                                    {{Form::label('permission'.$key,'Edit',['class'=>'custom-control-label'])}}<br>
                                                                </div>
                                                            @endif
                                                        @endif
                                                        @if(in_array('delete pipeline '.$pipeline->name,(array) $permissions))
                                                            @if($key = array_search('delete pipeline '.$pipeline->name,$permissions))
                                                                <div class="col-md-3 custom-control custom-checkbox">
                                                                    {{Form::checkbox('permissions[]',$key,$role->permission, ['class'=>'form-check-input crm_checkall isscheck_'.str_replace(' ', '', $pipeline->name),'id' =>'permission'.$key])}}
                                                                    {{Form::label('permission'.$key,'Delete',['class'=>'custom-control-label'])}}<br>
                                                                </div>
                ...
                <div class="tab-pane fade" id="inventory" role="tabpanel" aria-labelledby="pills-inventory-tab">
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            @if(!empty($permissions))
                                <h6 class="my-3">{{__('Assign Inventory Setup related Permission to Roles')}}</h6>
                                <table class="table table-striped mb-0" id="">
                                    <thead>
                                    <tr>
                                        <th>
                                    ...
                                    @foreach($modules as $module)
                                        <tr>
                                            ...
                                            <td><input type="checkbox" class="form-check-input align-middle ischeck inventory_checkall"  data-id="{{str_replace(' ', '', $module)}}" ></td>
                                            <td><label class="ischeck inventory_checkall" data-id="{{str_replace(' ', '', $module)}}">{{ ucfirst($module) }}</label></td>
                                            <td>
