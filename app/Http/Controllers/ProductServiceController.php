        ...
        if (\Auth::user()->can('create product & service')) {
            ...
            if ($request->hasFile('images')) {
                ...
                foreach ($request->file('images') as $image) {
                    ...
                    $path = $image->storeAs('uploads/pro_image', $fileName, 'public');
                    $images[] = $path;
                }
                $productService->images = json_encode($images);
            }

            // Handle additional fields
            $productService->product_type_code = $request->product_type_code;
            $productService->product_type_name = $request->product_type_name;
            $productService->group_name = $request->group_name;
            $productService->warehouse_name = $request->warehouse_name;
            $productService->tags = $request->tags;
            $productService->inventory = $request->inventory;
            $productService->minimum_stock = $request->minimum_stock;
            $productService->maximum_stock = $request->maximum_stock;
            $productService->price_after_tax = $request->price_after_tax;
            $productService->main_category = $request->main_category;
            $productService->sub_category = $request->sub_category;
            $productService->child_category = $request->child_category;
            $productService->tax_2 = $request->tax_2;
            $productService->status = $request->status;
            $productService->mother = $request->mother;
            $productService->date = $request->date;
            $productService->inventory_vendor = $request->inventory_vendor;
            $productService->expiration_date = $request->expiration_date;
            $productService->uom = $request->uom;
            $productService->temperature_inwards_delivery = $request->temperature_inwards_delivery;
            $productService->target_penjualan = $request->target_penjualan;
