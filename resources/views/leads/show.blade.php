...
@push('script-page')
    ...
    var showLeadUrl = "{{ route('leads.show', $lead->id) }}";
...
@section('breadcrumb')
    ...
    <div class="d-flex align-items-center mt-2">
        <a href="{{ route('leads.index') }}" class="text-black me-3">
            <i class="fas fa-arrow-left"></i> <!-- Font Awesome Left Arrow Icon -->
        </a>

        @if($lead->company_entity_logo)
        <img src="{{ asset('storage/app/public/' . $lead->company_entity_logo) }}" alt="Company Logo" style="width: 32px; height: 32px; object-fit: cover; border-radius: 4px;">
        @else
            <i class="fas fa-building fa-2x me-2"></i>
        @endif

        <div>
            <h3 class="d-inline">{{ $lead->company_entity_name }}</h3>
...
@section('action-btn')
    <div class="float-end">
        ...
        <a href="#" data-size="lg" data-url="" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{ __('Dot') }}" class="btn btn-sm btn-primary">
            <i class="fas fa-ellipsis-h"></i> <!-- Font Awesome Three Dot Horizontal Icon -->
        </a>
            @if($previousLead)
                <a href="{{ route('leads.show', $previousLead->id) }}" class="btn btn-sm btn-primary" title="{{ __('Previous Lead') }}">
                    <i class="fas fa-arrow-left"></i>
                </a>
            @endif

            @if($nextLead)
                <a href="{{ route('leads.show', $nextLead->id) }}" class="btn btn-sm btn-primary" title="{{ __('Next Lead') }}">
