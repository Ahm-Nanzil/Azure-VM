...
<script>
    ...
    myDropzone = new Dropzone("#dropzonewidget", {
        ...
        url: "{{route('customer.file.upload',$customer->id)}}",
    ...
    @if (file_exists(storage_path('customer_files/'.$file->file_path)))
    // Create the mock file:
    var mockFile = {name: "{{$file->file_name}}", size: {{\File::size(storage_path('customer_files/'.$file->file_path))}}};
    // Call the default addedfile event handler
    myDropzone.emit("addedfile", mockFile);
    // And optionally show the thumbnail of the file:
    myDropzone.emit("thumbnail", mockFile, "{{asset(Storage::url('customer_files/'.$file->file_path))}}");
    myDropzone.emit("complete", mockFile);

    dropzoneBtn(mockFile, {download: "{{route('customer.file.download',[$customer->id,$file->id])}}", delete: "{{route('customer.file.delete',[$customer->id,$file->id])}}"});
    @endif
    @endforeach
    ...
    $('.summernote-simple').on('summernote.blur', function () {
        ...
        $.ajax({
            url: "{{route('leads.note.store',$customer->id)}}",
...
<script>
    ...
    var showLeadUrl = "{{ route('customer.show', \Crypt::encrypt($customer->id)) }}";
        ...
        success: function (response) {
            ...
            $.ajax({
                ...
                error: function (xhr) {
                    ...
                    show_toastr('errror', 'There was an error to loading!');
                }
            });
        },
        error: function (xhr) {
...
function reinitializeDropzone() {
    Dropzone.autoDiscover = false; // Disable auto-discovery for Dropzone
    ...
    // Add event for sending CSRF token and customer ID
    myDropzone.on("sending", function (file, xhr, formData) {
        formData.append("_token", $('meta[name="csrf-token"]').attr('content'));
        formData.append("customer_id", {{$customer->id}});
    });

    // Recreate the files from the backend if any
    @foreach($customer->files as $file)
    @if (file_exists(storage_path('customer_files/'.$file->file_path)))
    // Create the mock file:
    var mockFile = {name: "{{$file->file_name}}", size: {{\File::size(storage_path('customer_files/'.$file->file_path))}}};
...
@section('breadcrumb')
    ...
    <div class="d-flex align-items-center mt-2">
        <a href="{{ route('customer.index') }}" class="text-black me-3">
        ...
        <div>
            <h3 class="d-inline">{{ $customer->company_entity_name }}</h3>
...
@section('action-btn')
    <div class="float-end">
        ...
        @can('edit customer')
            <a href="#" data-size="lg" data-url="{{ route('customer.edit',$customer['id']) }}" data-ajax-popup="true" title="{{__('Edit Customer')}}" data-bs-toggle="tooltip" data-original-title="{{__('Edit')}}" data-title="{{__('Edit')}}" class="btn btn-sm btn-primary">
